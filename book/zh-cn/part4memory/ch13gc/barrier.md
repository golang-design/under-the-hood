...
需要先厘清一个常见的混淆：垃圾回收里的**写屏障（write barrier）**，与并发编程里的
**内存屏障（memory barrier）**并不是一回事。内存屏障是 CPU 层面约束内存操作次序的机制
（见 [11.9 内存一致模型](../../part3concurrency/ch11sync/mem.md)）；而 GC 的写屏障，是编译器在
指针写操作处插入的一小段代码，用来在并发回收期间维持三色抽象的不变式。本节讨论的是后者。
