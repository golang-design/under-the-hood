...
找不到用户活儿干时被 `findRunnable`（[9.4 调度循环](../../part3concurrency/ch09sched/schedule.md)）顺手唤起。三者
...
标记还长出一层「按 span 扫描」的优化（Green Tea GC，go1.25 引入、go1.26 默认开启）：除了逐个
对象的灰色队列，`gcWork` 还维护一个待处理 **span 队列**（`spanq`），把同一个 span 内多个待扫对象
聚成一次批量扫描，改善了小对象密集场景下的访存局部性。它在数据结构上是对上面这套队列的扩充，灰色
对象的语义不变，本节按经典的「逐对象灰色队列」叙述，足以理解标记的骨架；按 span 的批量扫描作为一层
独立的性能演进，本章 [13.11 Green Tea](./greentea.md) 专门展开。
...
- **精确扫描的元数据演进**：从早期每 arena 一张全堆指针位图，到 go1.22 把指针布局收进 span 末尾
  与对象首字的「分配头」，目的是改善访存局部性、缩减元数据占用；go1.25 引入、go1.26 默认开启
  的 Green Tea GC（[13.11](./greentea.md)）再加一层按 span 的批量扫描，进一步优化小对象密集场景。
