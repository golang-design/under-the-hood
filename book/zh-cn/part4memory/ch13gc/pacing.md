...
```mermaid
flowchart LR
    T["trigger<br/>启动标记"] -->|标记进行中| G["H_goal<br/>期望此刻刚好标记完"]
    G --> A(["heap live →"])
...
3. Michael Knyszek. *Soft memory limit.* Go proposal 48409, 2022.
   ...
   （`GOMEMLIMIT` 的设计与软上限语义）。其早期渊源见 Brad Fitzpatrick. *runtime: mechanism
   for monitoring heap size.* Go issue #16843, 2016. https://github.com/golang/go/issues/16843
   与 Austin Clements. *runtime/debug: add SetMaxHeap API.* CL 46751, 2017.
   https://go-review.googlesource.com/c/go/+/46751 （软 / 硬堆上限语义最早的两次尝试）
