...
在保持本章其余部分基于 `go1.15` 的前提下，后续版本中的 GC 还出现了几个值得补记的演进节点。

### Go 1.19：软内存上限

Go 1.19 为运行时引入了软内存上限，可通过 `runtime/debug.SetMemoryLimit` 或环境变量 `GOMEMLIMIT` 进行设置。
这一限制同时考虑 Go 堆与运行时管理的其他内存，并与 `GOGC` 协同工作；即便在 `GOGC=off` 时，运行时仍会尽可能遵守这一软限制。
当程序总内存使用逐渐逼近该限制、继续严格维持限制可能导致 GC 反复震荡时，运行时还会将 GC 的 CPU 使用限制在大约 50%，以避免应用因过度 GC 而难以继续前进。

### Go 1.25：实验性 Green Tea GC

Go 1.25 开始提供实验性的 Green Tea GC，可在构建时通过设置 `GOEXPERIMENT=greenteagc` 启用。
它的设计重点是通过更好的局部性和 CPU 可扩展性来改善小对象的标记与扫描性能。
按照官方博客给出的数据，许多工作负载中的 GC 时间可减少约 10%，部分工作负载最高可达 40%，但并非所有场景都会获得同等收益。

### Go 1.26：Green Tea GC 默认启用

到了 Go 1.26，Green Tea GC 在吸收 Go 1.25 的反馈后默认启用。
除延续对小对象标记与扫描的改进外，在较新的 amd64 平台上，运行时还会在可能的情况下利用向量指令扫描小对象，从而进一步降低 GC 开销。
