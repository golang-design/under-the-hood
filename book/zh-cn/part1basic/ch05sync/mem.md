...
> 本节在讲清当前模型之后，会专门交代这次修订的来龙去脉（见 [5.9.8](#598-设计的演进)）。
...
重排无依赖的读写，这些对单线程完全正确的变换，足以让 5.9.1 那段程序失效。于是，
只有硬件的内存模型还不够，我们还需要一层语言自己的内存模型，把编译器也一并约束进来。

语言层的内存模型出奇地难做，它要同时安抚两个互相拉扯的诉求：给程序员一个可推理的语义，
又不堵死编译器与硬件的优化。工业界给出的答案，正是 5.9.3 讲过的那纸 DRF-SC 契约，
