...
## 18.1.4 绕开 cgo：把第一笔成本也削掉

前面说第一笔成本（cgo 边界穿越）「理论上可以绕开」。这并非空话，Go 生态里确有两条路。

一条是 **`purego`**（由 Ebitengine 项目维护）。它在运行时用 `dlopen`/`dlsym` 动态加载共享库，
再用一段手写汇编的跳板按平台 ABI 直接调用函数地址，从而**完全不经过 cgo 的代码生成**。它的动机
首先是工程性的：不依赖 C 编译器，于是保住了 15.6 里说 cgo 会折损的那几样东西，静态构建、
交叉编译、快构建。值得一提的是，`purego` 仍然要复用运行时的 `cgocall` 来完成栈切换与
`entersyscall`,`runtime/cgocall.go` 的注释里专门点了它的名。也就是说，它绕开的是 cgo 的
**构建期**代价，而非**运行期**那次边界穿越的状态转换,第一笔成本的核心部分依然要付。

另一条更彻底，但只在特定场景成立：当「C 库」其实是一个**远端服务**时，根本不必把它链进同一个
...
哪条路都说明同一件事：FFI 边界不是非此即彼的铁律，而是一个可以挪动的设计选择。你可以选择把它放在
**函数调用**的粒度（cgo/purego，最贴身、最快，但污染工具链），也可以放在**进程**的粒度
（IPC，最干净，但有序列化成本）。选在哪里，取决于调用有多频繁、数据有多大、以及你有多在乎那条
纯 Go 的工具链。
...
用图捕获把一整条命令序列压成一次跨界。再不济，还能把整个边界挪到进程之外。
...
6. 本书 [15.6 cgo](../../part5toolchain/ch15compile/cgo.md)、
