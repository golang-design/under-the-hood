...
goroutine，并用一套 M、P、G 三元组承载它。在动手分析调度算法（[9.4](./schedule.md) 起）之前，
...
（[14.3 栈的增长](../../part4memory/ch14stack/grow.md)）整段搬迁扩容。这把「有栈」的固定开销压到了与无栈协程可比的量级。
...
二者、携带本地资源的许可证。它们如何在每一次调度循环里协同运转，是 [9.4 调度循环](./schedule.md)
