...
这些用户看不见的工作也一并调度好；还要严格维持用户代码的内存次序（见 [11.9 内存一致模型](../ch11sync/mem.md)）。
...
或被抢占后，再取下一个。本地队列空了，就去别处找活干，这便引出了工作窃取（见 [9.2](./steal.md)）。
队列、窃取、线程管理、抢占等部件如何协作，是后续各节的内容。

## 9.1.3 P 是怎么来的：从 GM 到 GMP
...
P 增设了 `runnext` 槽，Go 1.14 加入了异步抢占，分别见 [9.3](./mpg.md) 与 [9.7](./preemption.md)）。
