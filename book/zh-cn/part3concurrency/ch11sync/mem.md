...
> Go 的内存模型在 2022 年随 Go 1.19 做过一次重要修订，
> 本节在讲清当前模型之后，会专门交代这次修订的来龙去脉（见 [11.9.8](#1198-设计的演进)）。
...
回到 11.9.1 那段会出错的程序。我们说过，写入 `data` 与置位 `done` 的次序，可能被硬件打乱。
...
生成相应的屏障与原子指令，把硬件的强弱差异吸收掉。换句话说，11.9.3 那笔「以免数据竞争换取
顺序一致」的交易，正是由工具链在这一层兑现的。读者只需面对一套模型，芯片的众声喧哗，
到不了你的代码面前。这就是内存模型存在的意义。

## 11.9.6 语言层的内存模型与工业界的教训

上一节谈的是硬件。可就算把代码跑在 x86 这样较强的芯片上，它仍可能出错，因为在硬件之上还有
一层会改动次序的角色：编译器。编译器的优化全都建立在「不改变单线程可观测行为」这一前提上，
它对多线程一无所知。把循环里对 `done` 的读取提到循环外、把变量长期留在寄存器、为优化而
重排无依赖的读写，这些对单线程完全正确的变换，足以让 11.9.1 那段程序失效。于是，
只有硬件的内存模型还不够，我们还需要一层语言自己的内存模型，把编译器也一并约束进来。

语言层的内存模型出奇地难做，它要同时安抚两个互相拉扯的诉求：给程序员一个可推理的语义，
又不堵死编译器与硬件的优化。工业界给出的答案，正是 11.9.3 讲过的那纸 DRF-SC 契约，
其思想可上溯到 Adve 与 Hill 1990 年的弱序定义。可是，把这纸契约真正写进一门语言，
历史证明是一连串的试错。

```mermaid
flowchart LR
    A["1990 弱序与 DRF<br/>Adve & Hill"] --> B["1996 Java 1.0 模型<br/>（有缺陷）"]
    B --> C["2004 JSR-133<br/>修复 Java 模型"]
    C --> D["2008 / 2011<br/>C/C++ 内存模型"]
    D --> E["2022 Go 1.19<br/>修订对齐 DRF-SC"]
```

最有名的前车之鉴是 Java。1996 年随 Java 1.0 发布的第一版内存模型是有缺陷的：volatile 变量
不具备同步语义，无法用来为周围的普通访问建立次序；final 字段在别的线程看来仍可能变化，
违背了不可变对象的初衷；模型还允许一些违反因果的荒谬结果。Java 社区花了数年，
通过 JSR-133 在 Java 5（2004）才把它重做：以 happens-before 为核心，给 final 字段明确的
可见性保证（前提是构造期间 `this` 不逃逸），对无数据竞争的程序保证顺序一致，
并为有竞争的程序定义有界、不违反因果的语义（Manson、Pugh 与 Adve 2005）。这段历史的教训
很直白：内存模型极难一次做对，连 Java 都先错了一版。

C 与 C++ 在 2011 年的标准里引入了更精细的模型，理论基础是 Boehm 与 Adve 2008 年的工作。
它把原子操作分出多个内存序档位：顺序一致（`seq_cst`）、获取与释放（`acquire`/`release`）、
松弛（`relaxed`），外加一个意图更弱的 `consume`。这套机制极其强大，能贴着硬件榨取性能，
也极难用对。C/C++ 还采取了「无数据竞争则顺序一致，否则一切免谈」的立场：任何数据竞争都是
未定义行为。

> 一处至今未解的难题：松弛原子带来了所谓凭空出现（out-of-thin-air, OOTA）的值。标准想禁止
> 这种自我论证、凭空捏造出来的结果，却发现任何足够严格的禁止条款，都会顺带禁掉人们想要的
> 优化。直到今天，C++ 标准也只能给出一句非规范的「建议」，拿不出精确的定义（见 WG21 文件
> P1217）。与之相关，`memory_order_consume` 因为没有编译器能按规范实现（实现都把它退化成了
> `acquire`），已被官方劝退（P0371）。这一角，是内存模型领域公认最硬的骨头，我们在
> [11.9.9](#1199-前沿与开放问题) 还会回到它。

尽管细节各异，C、C++、Java、JavaScript、Rust、Swift 与 Go 最终都汇聚到同一个承诺上：
无数据竞争的程序顺序一致。它们的分野在于如何对待有竞争的程序。C 与 C++ 选择未定义行为，
编译器可以任意发挥；Java、JavaScript 与 Go 则把结果限定在有界范围内。Go 站在后者一边，
明确拒绝未定义行为，哪怕程序写错了，行为也是有界、可调试的。带着工业界这些教训，
接下来再看 Go 自己的内存模型，就能读懂它每一处取舍的来由。
...
程序的结果也就能由某个顺序一致的交错执行解释，这正是 11.9.3 所说的 DRF-SC（其证明与 Boehm
...
Go 的内存模型在工程上已经落定，日常使用它，并不需要再往下读。但这纸契约底下的科学远未完结。
这一节为有兴趣的读者，勾勒内存模型研究的几处前沿。它们大多还是学术提案，尚未进入任何语言标准。

绕不开的仍是上一节提到的凭空出现（OOTA）问题。如何给松弛原子一个既禁止凭空之值、
又不妨碍优化的语义，是这一领域多年来最硬的核心。围绕它，近十年出现了几条有代表性的路线。

```mermaid
flowchart TD
    OOTA["开放难题：凭空出现 OOTA<br/>既要禁止凭空之值，又不妨碍优化"]
    OOTA --> P["承诺语义 Promising<br/>承诺 + 线程局部证明"]
    OOTA --> R["RC11<br/>修补 C11 顺序一致原子"]
    OOTA --> L["Linux 内核内存模型 LKMM<br/>可执行，已进入生产"]
    OOTA --> I["Iris / iRC11<br/>弱内存的程序逻辑与验证"]
```

一条路线是承诺语义（Promising Semantics，Kang 等人 2017）。它允许一个线程先对未来的某个写
作出承诺，让别的线程提前读到，但每个承诺都要配一份线程局部的证明，表明这个写即便没有承诺
也终能产生，以此堵死自我论证的循环。它被认为是第一个同时做到「禁止 OOTA」与「保留优化」的
模型；后续的 Promising 2.0（2020）补上了它原先无法支持的若干全局优化。需要强调，
这是研究提案，并未被任何标准采纳。

另一条路线是修补现有标准。Lahav 等人 2017 年指出，C11 中顺序一致原子的语义有缺陷，
按标准方案编译到 POWER 上甚至是不健全的；他们提出的 RC11 修好了这一点，并保住了 DRF-SC。
今天的学术工作多以 RC11 作为 C11 的基准模型。

也有人把模型直接做进工程。Linux 内核内存模型（LKMM，Alglave 等人 2018）是一个可被工具执行
的形式化模型，用 cat/herd 写成，覆盖 `READ_ONCE`、各类屏障乃至 RCU，真实地躺在内核源码树
（`tools/memory-model`）里，供内核开发者用石蕊测试核对自己的并发代码。它是少数从论文走进
生产的内存模型之一。

还有一条路是为弱内存程序建立可机器检验的证明体系。从 GPS（2014）到基于 Iris 框架的 iRC11，
研究者把分离逻辑、幽灵状态与协议带进了弱内存，能够形式化地验证并发库的正确性，
并在此过程中发现过 Rust 标准库 `Arc` 里的一处数据竞争。

把这些放在一起看，会得到一个朴素却有力的结论：内存模型这件事，连最顶尖的研究者都还在为它的
基础部分较劲。Go 之所以只给出一套简单、且只暴露顺序一致原子的模型，背后正有这门学科的难度
作注脚。下一节，我们就把这份简单背后的工程权衡，连同 Go 给用户的忠告，一并说清楚。
...
为帮助读者亲手体会 11.9.1 中存储缓冲带来的反直觉一幕，下面给出一个交互演示。它模拟内存模型
