...
> 之后，会同时交代旧设计与这次重写的来龙去脉（见 [5.2.4](#524-go-124-的-swiss-table-重写)）。
...
把那条发散曲线压平。Swiss Table 正是这条路线上的集大成者（[5.2.3](#523-swiss-table-设计abseil)）。
...
（[5.2.4](#524-go-124-的-swiss-table-重写)），同一进程内不同 `map` 的哈希也彼此独立。这一层
...
查找的内层就是 [5.2.3](#523-swiss-table-设计abseil) 描述的并行匹配。`matchH2` 是其核心，amd64
