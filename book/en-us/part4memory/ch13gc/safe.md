...
Placing this line into the lineage: precise GC is another expression of "deep cooperation between the compiler and the runtime" ([3.2](../../part1overview/ch03life/compile.md)): GC is not a standalone library that can be bolted onto any language, it and the compiler share a convention about "where the pointers are." This cooperation is a cost, and also a backbone: precisely because the runtime knows exactly where every pointer is, Go has room to keep refactoring its collector (from tricolor marking to the hybrid write barrier, and on toward new directions such as Green Tea ([13.11](./greentea.md)), see [13.1](./basic.md) and [13 Overview](./readme.md)) without worrying that touching the object layout breaks correctness. Precision is the premise that lets all of this optimization proceed safely.
