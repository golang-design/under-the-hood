...
// 关于一般综述，参见 malloc.go。
//
// 本文件考虑分配策略。所有的空闲 span 都保存在一个 treap 中。
// 有关概述，请参阅 https://en.wikipedia.org/wiki/Treap 或
// https://faculty.washington.edu/aragon/pubs/rst89.pdf。
// sema.go 也有一个 treap 的实现。
//
// 每个 treapNode 都包含一个 span。treap 根据基地址进行排序，且每个 span 一定有一个唯一的基地址。
// span 根据 first-fit 算法进行返回，获得具有最低基地址且满足要求的 span。
//
// first-fit 算法之所以可行，是因为每个 streapNode 的增加，以保持在该 streapNode 的子树中最大跨度的大小，
// 下面我们将这个不变性称为 maxPages 不变性。
//
// 主要操作包括：
...
//
// 无论何时当一个指向 span 的、被 treap 持有的指针被需要时，
// 这个指针必须不能被修改。如需修改，则先从 treap 中移出。
//
