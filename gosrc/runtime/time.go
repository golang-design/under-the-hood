...
// 在此文件外部的代码使用计时器时必须非常小心。
//
// pp, status 和 nextwhen 字段只能由该文件的代码使用
//
// 创建新计时器值的代码可以设置 when, period, f, args, 和 seq 字段
// 可以将新的计时器传递给 addtimer (由 time.startTimer 调用)
// 执行该操作后，不应修改任何字段
//
// 一个活跃的计时器（已经传递给 addtimer 的计时器）可能在不活跃后传递给 deltimer(time.stopTimer)，
// 之后不再是活跃的计时器，成为非活跃计时器。
// 在非活跃计时器中，可以修改 period, f, arg, 和 seq 字段，但不能修改 when 字段。
// 丢弃一个非活跃计时器并让 GC 进行收集是允许的，将非活跃计时器传递给 addtimer 是不正确的。
// 只有新分配的计时器可以传递给 addtimer。
//
// 可以将活跃计时器传递给 modtimer，不允许修改任何字段，并且计时器将保持活跃
//
// 可以将不活跃的计时器传递给 resettimer 将其变更为带有更新的 when 字段的活跃计时器
//
// 计时器的操作包括 addtimer, deltimer, modtimer, resettimer, cleantimers,
// adjusttimers 和 runtimer.
//
// 我们不允许同时调用 addtimer/deltimer/modtimer/resettimer，但是 adjusttimers 和
// runtimer 可以与其中任何一个同时调用。
//
// 活跃计时器生存在计时器字段中并附着在 P 上，非活跃计时器在该位置仅临时保存到被移除为止。
//
// addtimer:
...
// 堆维护算法。
// 该算法手动检查切片索引错误。
// 如果程序正在竞争访问计时器，则会发生切片索引错误消
// 我们不想在这里 panic，因为它将输出神秘的 panic holding locks 息
// 相反，我们在没有锁的情况下 panic
...
// See issue #25686.
// 如果 timer 数据结构因为程序竞争被破坏，则调用 badTimer
// 我们在此处 panic 而不是在切片访问时 panic。
// 见 issue #25686.
func badTimer() {
